...
module.exports = async ({vulnerabilities, numVulnerabilities }) => {
    ...
    await exec.exec('git', ['config', 'user.email', "bourgoin.theo1@gmail.com"]);
    await exec.exec('git', ['config', 'user.name', "Psyker"]);
    await exec.exec('git', ['push', 'origin', 'security-js-fix']);
